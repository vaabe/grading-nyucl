...
- `-s` to save current grading progress. A script is run that parses the comments files for all students, saves their comments in the comments buffer, and save their grades in the grades file 
	+ N.B. the grades are saved in a "tmp" file, file `grades-tmp.csv`, not the original grades.csv file. This is so a copy of the original grades file format is preserved (in case anything weird happens). 
