...
- `-s` to save current grading progress. A script is run to parse the comments files for all students, save their comments in the comments buffer, and save their grades in the grades file 
	+ N.B. the grades aren't saved in a file called grades-tmp.csv, not the original grades.csv file. This is so a copy of the original grades file format is preserved (in case anything weird happens). 
